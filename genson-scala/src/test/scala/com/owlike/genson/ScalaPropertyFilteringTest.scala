...
class ScalaPropertyFilteringTest extends FunSuite with Matchers {
  ...
  test("test filtering out a property across all classes") {
  ...
  test("test filtering out a property for specific class") {
  ...
  test("test renaming a property across all classes") {
    val genson = new ScalaGenson(new GensonBuilder().withBundle(ScalaBundle()).rename("amount","quantity").create())
    val expected = """{"quantity":10,"date":100,"name":"Hi"}"""
